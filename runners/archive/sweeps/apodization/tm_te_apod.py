"""
Study: apodization sweep for the pi-shift Bragg grating — TE vs TM.

Sweep the number of apodized teeth per side (each side of the pi-shift cavity)
for both polarizations, on the same baseline device used by the TM/TE comparison
(runners/tm/_tm_vs_te_common.build_base_cfg). Apodization ramps the corrugation
depth from a small floor (center_mod_depth_nm) at the cavity up to the full
corrugation depth (300 nm) over n_apod_periods_each_side teeth (linear profile).
...
Run locally (sequential):
    python -m runners.sweeps.tm_te_apod

Run on Athena as a parallel SLURM array (one task per cartesian point):
    bash athena/deploy_athena.sh --option3 --spec=runners.sweeps.tm_te_apod

Outputs land in <BASE_SAVE_DIR>/tm_te_apod/results/ on Athena (athena_run_one.py
sets RUN_NAME from the module short name) and sync to
results_from_athena/tm_te_apod/results/ via `--results-no-fsp`.
"""
