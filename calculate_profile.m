...
function [env, x_pks, y_pks] = calculate_envelope(x, y)
    ...
    if length(pks) < 2, env = y; x_pks=[]; y_pks=[]; return; end
    
    x_pks = locs(:); y_pks = pks(:);
    y_start = interp1(x_pks(1:2), y_pks(1:2), x(1), 'linear', 'extrap');
    y_end   = interp1(x_pks(end-1:end), y_pks(end-1:end), x(end), 'linear', 'extrap');
    
    x_nodes = [x(1); x_pks; x(end)];
    y_nodes = [y_start; y_pks; y_end];
    env = interp1(x_nodes, y_nodes, x, 'pchip');
end
