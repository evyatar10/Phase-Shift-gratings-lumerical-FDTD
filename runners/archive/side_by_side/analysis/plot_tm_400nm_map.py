"""
Build the device-1 deliverable maps for the TM side-by-side scan
(runners/side_by_side/side_by_side_tm_400nm.py). TM analog of plot_te_300nm_map.py.
...
heatmap over (Δy gap, Δx stagger). Q (= λ_res / |spectral_fwhm_nm|), the spectral
FWHM, and the resonance wavelength are plotted alongside as records.
...
the .mat itself (not parsed from the filename). Robust to missing tasks.
    ...
    python -m runners.side_by_side.analysis.plot_tm_400nm_map
"""
...
def main():
    ...
    heatmap(ax, T, gaps, stags, "Device-1 PEAK TRANSMISSION at resonance (TM, 400 nm corr)",
            "viridis", "{:.3f}", "peak T = |S21|²")
    fig.tight_layout()
    p1 = os.path.join(OUT_DIR, "map_transmission_TM.png")
    ...
    fig.suptitle("Device-1 resonance records — TM, 400 nm corrugation, N=80")
    fig.tight_layout()
    p2 = os.path.join(OUT_DIR, "map_records_TM.png")
