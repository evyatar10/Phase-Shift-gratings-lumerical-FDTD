"""
Diagnostic variant: check_gradient with smaller FD step (dx=10 nm).

Hypothesis: with dx=50 nm we may be picking up nonlinearities in FOM(p) that
break the small-perturbation assumption of FD. A smaller step (dx=10 nm,
matching mesh-quarter perturbation) would give cleaner FD signal.

If FD ≈ adjoint here but not at dx=50 → FD was sensing nonlinearity, true
gradient closer to adjoint.
If FD still disagrees by similar factor → FD step size isn't the issue.
"""
...
SPEC = InverseDesignSpec(
    ...
    mode                  = "check_gradient",
    check_gradient_dx_nm  = 10.0,           # ← small FD step
