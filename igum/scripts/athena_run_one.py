...
# IGUM runs Lumerical NATIVELY (no container): all paths derive from WORK_DIR,
# exported by the job script (falls back to the container-style /work layout).
...
config.BASE_SAVE_DIR  = os.path.join(WORK_DIR, 'results')
config.NEFF_DATA_PATH = os.path.join(WORK_DIR, 'data', 'FDE_sweep_results.mat')
config.USE_GPU        = True
config.LUMAPI_PATH    = os.environ.get(
    'LUMAPI_PATH', '/opt/lumerical/v261/api/python/lumapi.py')
...
SWEEP_LIST  = os.environ.get("SWEEP_LIST",
                             os.path.join(WORK_DIR, "data", "sweep_list.txt"))
...
def _run_kind_spec(line: str) -> None:
    ...
    if locked_path:
        ...
        # Legacy study modules hardcode container-style /work/... sidecar
        # paths; IGUM runs natively, so remap the prefix onto WORK_DIR.
        if locked_path.startswith("/work/") and WORK_DIR != "/work":
            locked_path = os.path.join(WORK_DIR, locked_path[len("/work/"):])
...
def _run_kind_inverse_design(line: str) -> None:
    ...
    # Apply Athena-runtime overrides on top of the spec's base config.
